...
namespace TravelCompanyFileImplement.Implements
{
    public class TravelStorage : ITravelStorage
    {
        ...
        public TravelViewModel GetElement(TravelBindingModel model)
        {
            ...
            var travel = source.Travels.FirstOrDefault(rec => rec.TravelName == model.TravelName || rec.Id == model.Id);
        ...
        private TravelViewModel CreateModel(Travel travel)
        {
            return new TravelViewModel
            {
                ...
                Price = travel.Price,
                TravelConditions = travel.TravelConditions.ToDictionary(recPC => recPC.Key, recPC => 
                (source.Conditions.FirstOrDefault(recC => recC.Id == recPC.Key)?.ConditionName, recPC.Value))
            };
        }
    }
}
